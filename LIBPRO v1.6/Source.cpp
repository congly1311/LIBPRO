...
int Chucnang(int hientai, NguoiDung ngDung[], int quyen, int &m, Quyen q[], char tdn[], Sach sach[], MuonSach muonsach[]) {
...
int main() {
	...
	quyen[19].stt = 20; quyen[20].Admin = quyen[20].ThuThu = quyen[20].DocGia = 1; strcpy(quyen[20].tenQuyen, "Dang xuat.");
	/*
	fstream inFile("Quyen.DAT", ios::in);
	int dem=0;
	while (!inFile.eof()){
	...
	while (true) {
		...
		do {
			...
			case 1: {
						...
						cout << "|________________________________________________________________|" << endl;
						cout << "Nhap Lua chon: "; cin >> op;
						kt = Chucnang(taikhoanhientai, ngDung, op, m, quyen, tdn, sach, muonsach);
						break;
			}
			case 2: {
						cout << "__________________________________________________________________" << endl;
